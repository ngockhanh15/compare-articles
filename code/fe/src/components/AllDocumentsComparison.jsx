...
import { getDetailedAllDocumentsComparison, checkDocumentSimilarity } from '../services/api';
...
const AllDocumentsComparison = () => {
  ...
  const getCurrentDocumentDuplicateRate = () => {
    ...
    return data?.currentDocument?.duplicatePercentage || data?.currentDocument?.duplicateRate || 0;
  };
  ...
  const getMatchingDocuments = () => {
    ...
    if (latestComparisonResult?.result?.sources && latestComparisonResult.result.sources.length > 0) {
      ...
      const mergedDocuments = originalDocuments.map(originalDoc => {
        ...
        const matchingSource = latestSources.find(source => 
        ...
        if (matchingSource) {
          const newDuplicateRate = matchingSource.duplicatePercentage || matchingSource.duplicateRate || originalDoc.duplicatePercentage || originalDoc.duplicateRate || 0;
          return {
            ...originalDoc,
            duplicatePercentage: newDuplicateRate,
            duplicateRate: newDuplicateRate,
            status: newDuplicateRate >= 50 ? 'high' : 
                    newDuplicateRate >= 25 ? 'medium' : 'low',
      ...
      const newDocuments = latestSources.filter(source => {
        return !originalDocuments.some(originalDoc => 
          source.documentId === originalDoc.documentId || 
          source.fileName === originalDoc.fileName ||
          source.id === originalDoc.id
        );
      }).map(source => {
        const duplicateRate = source.duplicatePercentage || source.duplicateRate || 0;
        return {
          ...source,
          duplicatePercentage: duplicateRate,
          duplicateRate: duplicateRate,
          status: duplicateRate >= 50 ? 'high' : 
                  duplicateRate >= 25 ? 'medium' : 'low',
          // Đảm bảo có các fields cần thiết với giá trị mặc định
          fileSize: source.fileSize || 0,
          fileType: source.fileType || 'Unknown',
          author: source.author || 'Unknown',
          uploadedAt: source.uploadedAt || source.createdAt || new Date().toISOString(),
          updatedFromTree: true,
          isNewFromTree: true
        };
      });
  ...
  const sortedAndFilteredDocuments = getMatchingDocuments().length > 0 ? 
  ...
  console.log('Latest duplicate percentage:', latestComparisonResult?.result?.duplicatePercentage);
  console.log('Final duplicate rate used:', getCurrentDocumentDuplicateRate());
  console.log('Original text length:', data?.currentDocument?.originalText?.length);
  console.log('Highlighted text length:', data?.currentDocument?.highlightedText?.length);
  console.log('Latest highlighted text length:', latestComparisonResult?.result?.highlightedText?.length);
  console.log('Has highlighted text:', hasHighlightedText());
  console.log('Current highlighted text preview:', getCurrentHighlightedText()?.substring(0, 200) + '...');
  console.log('Highlighted segments:', data?.highlightedSegments);
  console.log('Raw matching documents:', data?.matchingDocuments);
  console.log('Latest sources from tree:', latestComparisonResult?.result?.sources);
  console.log('Using latest documents:', latestComparisonResult?.result?.sources?.length > 0);
  console.log('Matching documents used:', getMatchingDocuments());
  console.log('First matching document duplicate fields:', data?.matchingDocuments?.[0] ? {
          ...
          <div className="grid gap-4 md:grid-cols-4">
            ...
            <div className="p-4 border border-neutral-200 rounded-xl bg-neutral-50">
              <div className={`text-2xl font-bold ${
                getCurrentDocumentDuplicateRate() >= 50 ? 'text-red-600' : 
                getCurrentDocumentDuplicateRate() >= 25 ? 'text-yellow-600' : 'text-green-600'
              }`}>
            ...
            <div className="flex items-center">
              ...
              <div>
                <div className="text-2xl font-bold text-blue-600">
                  {latestComparisonResult?.result?.totalMatches || data?.displayedMatches || data?.totalMatches || 0}
                  {(latestComparisonResult?.result?.hasMoreMatches || data?.hasMoreMatches) && 
                    <span className="text-sm text-neutral-500">
                      /{latestComparisonResult?.result?.totalDocumentsInSystem || data?.totalMatches}
                    </span>
                  }
                </div>
                <div className="text-sm text-neutral-600">
                  ...
                  {(latestComparisonResult?.result?.hasMoreMatches || data?.hasMoreMatches) && 
                    <div className="text-xs text-orange-600">Hiển thị top matches</div>
                  }
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                <div className="text-2xl font-bold text-red-600">
                  {sortedAndFilteredDocuments.filter(doc => (doc.duplicatePercentage || doc.duplicateRate || 0) > 50).length}
                </div>
              ...
              {data?.currentDocument?.originalText ? (
                <div className="p-4 border rounded-lg border-neutral-200 bg-neutral-50">
                  {hasHighlightedText() ? (
                    ...
                    <div 
                      ...
                      dangerouslySetInnerHTML={{ __html: getCurrentHighlightedText() }}
                      style={{
                        lineHeight: '1.8'
                      }}
                    />
                    ...
                    <div>
                      ...
                      {(data?.matchingDocuments?.length > 0 || getCurrentDocumentDuplicateRate() > 0) && (
                          ...
                          <div className="flex items-center">
                            ...
                            <div>
                              <h4 className="text-sm font-semibold text-yellow-800">
                                {latestComparisonResult ? 'Đang xử lý highlight...' : 'Chưa thể tô màu nội dung trùng lặp'}
                              </h4>
                              <p className="text-sm text-yellow-700">
                                {latestComparisonResult ? 
                                  'Đã lấy được kết quả so sánh mới nhất từ cây, đang cập nhật highlight...' :
                                  `Tìm thấy ${data?.matchingDocuments?.length || 0} documents trùng lặp nhưng chưa thể highlight các đoạn text cụ thể.`
                                }
                              </p>
                            </div>
                          </div>
                        ...
                        <div>
                          ...
                          <p className="text-sm text-blue-700">
                            {latestComparisonResult?.result?.matches?.length > 0 ? (
                              <>
                                Đã tô màu <strong>{latestComparisonResult.result.matches.length}</strong> đoạn văn bản trùng lặp từ <strong>{latestComparisonResult.result.totalMatches || data?.matchingDocuments?.length || 0}</strong> documents.
                                <br />
                                <span className="text-xs">
                                  Kết quả được cập nhật từ cây so sánh mới nhất. Di chuột lên đoạn được tô màu để xem thông tin chi tiết.
                                </span>
                              </>
                            ) : data?.highlightedSegments?.length > 0 ? (
                              <>
                                Đã tô màu <strong>{data.highlightedSegments.length}</strong> đoạn văn bản trùng lặp từ <strong>{data.matchingDocuments.length}</strong> documents.
                                <br />
                                <span className="text-xs">
                                  Các đoạn được tô màu có độ tương tự từ 5% trở lên. Di chuột lên đoạn được tô màu để xem thông tin chi tiết.
                                </span>
                              </>
                            ) : (
                              <>
                                Đã tô màu các từ khóa chung từ <strong>{data?.matchingDocuments?.length || 0}</strong> documents trùng lặp.
                                <br />
                                <span className="text-xs">
                                  Các từ được tô màu là những từ khóa xuất hiện trong cả văn bản gốc và documents trùng lặp.
                                </span>
                              </>
                            )}
                          </p>
                ...
                <div className="py-12 text-center">
                  ...
                  <div className="p-4 mt-4 text-left bg-gray-100 rounded-lg">
                    <h4 className="mb-2 font-semibold text-gray-800">Debug Information:</h4>
                    <div className="space-y-1 text-xs text-gray-600">
                      <div>Data exists: {data ? 'Yes' : 'No'}</div>
                      <div>Current document exists: {data?.currentDocument ? 'Yes' : 'No'}</div>
                      <div>Original text exists: {data?.currentDocument?.originalText ? 'Yes' : 'No'}</div>
                      <div>Original text length: {data?.currentDocument?.originalText?.length || 0}</div>
                      <div>Highlighted text exists: {data?.currentDocument?.highlightedText ? 'Yes' : 'No'}</div>
                      <div>Highlighted text length: {data?.currentDocument?.highlightedText?.length || 0}</div>
                      <div>Matching documents count: {data?.matchingDocuments?.length || 0}</div>
                      <div>Highlighted segments count: {data?.highlightedSegments?.length || 0}</div>
                    </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {data?.highlightedSegments && data.highlightedSegments.length > 0 ? (
                      ...
                      data?.matchingDocuments?.slice(0, 10).map((doc, index) => {
                        const colors = [
                          '#ef4444', '#f97316', '#eab308', '#22c55e', '#06b6d4', 
                          '#3b82f6', '#8b5cf6', '#ec4899', '#f43f5e', '#84cc16'
                        ];
                        ...
                        return (
                          <div key={`legend-common-${doc.id}-${index}`} className="flex items-center text-xs">
                            <div 
                              className="w-4 h-4 mr-2 border rounded"
                              style={{ backgroundColor: backgroundColor, borderColor: color }}
                            />
                            <span className="truncate text-neutral-600" title={doc.fileName}>
                              {doc.fileName}
                            </span>
                          </div>
                        );
